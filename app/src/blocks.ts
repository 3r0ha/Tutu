/**
 * Блоки лендинга-приглашения (зеркало серверной модели).
 *
...
export interface StayBlock extends BaseBlock {
  kind: 'stay';
  heading: string;
  /** Порядок, в котором гостю показывать другие варианты. */
  sort?: 'rating' | 'price';
 ...
 *
 * Свадьба, корпоративный выезд и сборы секции — разные страницы: одной нужна
 * программа дня и жильё, другой — контакты координатора и сухой тайминг.
 * Шаблон только расставляет блоки, дальше всё правится руками.
 */
...
const id = (name: string): string => `b-${name}-${Math.random().toString(36).slice(2, 7)}`;
...
export const TEMPLATES: Template[] = [
  {
    ...
    cover: '💍',
    build: () => [
      { id: id('cover'), kind: 'cover', cover: '💍', title: '', subtitle: 'Будем рады видеть вас' },
      { id: id('text'), kind: 'text', heading: 'О дне', body: '' },
      {
        id: id('route'), kind: 'route',
        heading: 'Как до нас добраться',
        lead: 'Выберите город — маршрут уже посчитан и покажется сразу.',
      },
      {
        id: id('schedule'), kind: 'schedule', heading: 'Программа',
        items: [
          { time: '15:00', text: 'Сбор гостей' },
          { time: '16:00', text: 'Церемония' },
          { time: '18:00', text: 'Ужин' },
        ],
      },
      {
        id: id('stay'), kind: 'stay', heading: 'Где остановиться', sort: 'rating',
        hotelName: '', address: null, rating: null, reviewCount: null, price: null,
        nights: null, quote: null, quoteDate: null, url: null,
      },
      { id: id('rsvp'), kind: 'rsvp', heading: 'Придёте?', lead: '' },
    ],
  },
  {
    key: 'corporate',
    label: 'Корпоративный выезд',
    cover: '🏔',
    build: () => [
      { id: id('cover'), kind: 'cover', cover: '🏔', title: '', subtitle: '' },
      {
        id: id('route'), kind: 'route',
        heading: 'Дорога',
        lead: 'Выберите город — покажем маршрут и стоимость.',
      },
      {
        id: id('schedule'), kind: 'schedule', heading: 'Расписание',
        items: [
          { time: '10:00', text: 'Заезд и размещение' },
          { time: '12:00', text: 'Рабочая сессия' },
        ],
      },
      {
        id: id('stay'), kind: 'stay', heading: 'Проживание', sort: 'price',
        hotelName: '', address: null, rating: null, reviewCount: null, price: null,
        nights: null, quote: null, quoteDate: null, url: null,
      },
      { id: id('contacts'), kind: 'contacts', heading: 'Координатор', body: '' },
      { id: id('rsvp'), kind: 'rsvp', heading: 'Подтвердите участие', lead: '' },
    ],
  },
  {
    key: 'sport',
    label: 'Сборы и соревнования',
    cover: '⚽️',
    build: () => [
      { id: id('cover'), kind: 'cover', cover: '⚽️', title: '', subtitle: '' },
      {
        id: id('route'), kind: 'route',
        heading: 'Как добраться',
        lead: 'Выберите город, откуда едете.',
      },
      {
        id: id('text'), kind: 'text', heading: 'Что взять с собой', body: '',
      },
      { id: id('contacts'), kind: 'contacts', heading: 'Тренер', body: '' },
      { id: id('rsvp'), kind: 'rsvp', heading: 'Едете?', lead: '' },
    ],
  },
];
